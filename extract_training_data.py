...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for k in range(0, len(words)):
            if pos[k] in switch:
                words[k] = switch[pos[k]]

        for s in range(-1, -4, -1):
            if abs(s) <= len(state.stack):
                stack_word = state.stack[s]
                if words[stack_word] in self.word_vocab:
                    out.append(self.word_vocab[words[stack_word]])
                else:
                    out.append(2)
            else:
                out.append(4)
        ...
        for b in range(-1, -4, -1):
            if abs(b) <= len(state.buffer):
                buf_word = state.buffer[b]
                if words[buf_word] in self.word_vocab:
                    out.append(self.word_vocab[words[buf_word]])
                else:
                    out.append(2)
            else:
                out.append(4)
    ...
    def get_output_representation(self, output_pair):
        return to_categorical(extractor.output_labels[output_pair], num_classes=91)
